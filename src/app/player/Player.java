...
public final class Player {
    ...
    @Getter
    @Setter
    private boolean shouldInsertAd = false;
    @Getter
    @Setter
    ...
    @Getter
    @Setter
    private AudioFile ad = null;
    @Getter
    @Setter
    private PlayerSource beforeAdSource = null;
    ...
    public void next(final User user) {
        if (shouldInsertAd) {
            beforeAdSource = new PlayerSource(source);
            source.setAudioFile(Admin.getAd());
            source.setAudioCollection(null);
            source.setRemainedDuration(Admin.getAd().getDuration());
            comeBack = true;
            shouldInsertAd = false;
            Admin.getInstance().giveMoneyFree((double) user.getAdPrice(), user);
            user.getAdAdministration().clearLists();
            user.setAdPrice(0);
            return;
        }
        if (comeBack) {
            source.setAudioFile(beforeAdSource.getAudioFile());
            source.setAudioCollection(beforeAdSource.getAudioCollection());
            source.setRemainedDuration(source.getDuration());
            comeBack = false;
        }
